...
bool AsyncCallbackWebHandler::canHandle(AsyncWebServerRequest *request) const {
  Serial.print("URI: ");Serial.print(_uri);Serial.print(" for method ");Serial.println((int) _method);
  Serial.print("Request method: ");Serial.println((int) request->method());

  if (!_onRequest || !request->isHTTP() || !(_method & request->method())) {
    !_onRequest && Serial.println("!onRequest");
    !request->isHTTP() && Serial.println("!request->isHTTP()");
    !(_method & request->method()) && Serial.println("!(_method & request->method())");
    return false;
  }
#ifndef ASYNCWEBSERVER_REGEX
  Serial.println("ASYNCWEBSERVER_REGEX is not defined");
#endif

#ifdef ASYNCWEBSERVER_REGEX
  Serial.println("ASYNCWEBSERVER_REGEX is defined");
  if (_isRegex) {
    Serial.println("URI is regex");
    std::regex pattern(_uri.c_str());
    std::smatch matches;
    std::string s(request->url().c_str());
    if (std::regex_search(s, matches, pattern)) {
      for (size_t i = 1; i < matches.size(); ++i) {  // start from 1
        Serial.print("Match ");Serial.print(i);Serial.print(": ");Serial.println(matches[i].str().c_str());
        request->_addPathParam(matches[i].str().c_str());
      }
    } else {
      Serial.print("Regex did not match ");Serial.println(request->url());
      return false;
    }
  } else
#endif
