...
void PEQProcessor::connectAudio(AudioStream& input, AudioStream& output, int inputChannel, int outputChannel) {
  ...
  // Create new connections in series
  connections[0] = new AudioConnection(input, inputChannel, biquadFilters[0], 0);
  connectionCount++;
  ...
  connections[MAX_PEQ_BANDS] = new AudioConnection(biquadFilters[MAX_PEQ_BANDS-1], 0, output, outputChannel);
  connectionCount++;
}
