...
float PEQProcessor::calculateMaxEqBoost(const PEQBand* currentBands, int numBands) const {
  float maxBoost = 0.0f;
  // Iterate over a more dense range of frequencies logarithmically
  const int numSamples = 100; // Increased number of samples
  for (int i = 0; i < numSamples; ++i) {
    float freq = 20.0f * pow(20000.0f / 20.0f, (float)i / (numSamples - 1)); // Logarithmic scale from 20Hz to 20kHz
